...
function pingEndpoints() {
  ...
  // Commented out successful ping notifications:
  ...
  Shelly.call("http.get", { url: target, timeout: CONFIG.httpTimeout }, function(response, error_code, error_message) {
    ...
    if (error_code !== 0 || !response || response.code !== 200) {
      ...
      // Successful ping notifications are commented out:
    ...
    if (failCounter >= CONFIG.numberOfFails) {
      ...
      if ((now - lastRebootTime) > CONFIG.rebootCooldown) {
        sendTelegramMessage(deviceName + " (" + location + "): " + emoji.reboot + "+Too+many+failures.+Rebooting+modem+at+" + getReadableTimestamp());
        lastRebootTime = now;
...
function startAfterDelay() {
  let timestamp = getReadableTimestamp();
  sendTelegramMessage(deviceName + " (" + location + "): " + emoji.ok + "+Modem+Ping+Watchdog+started+at+" + timestamp);
  pingLoopActive = true;
  jitteredPingLoop();
}
...
function checkNetworkReadyAndStart() {
  ...
  if (!status || !status.sta_ip || status.sta_ip === "") {
    print("Modem: Still waiting for Wi-Fi...");
    Timer.set(2000, false, checkNetworkReadyAndStart);
    return;
  }
  let timestamp = getReadableTimestamp();
  print("Modem: Wi-Fi is ready with IP " + status.sta_ip);
...
Shelly.addStatusHandler(function (status) {
  ...
  let timestamp = getReadableTimestamp();
  sendTelegramMessage(deviceName + " (" + location + "): " + emoji.ok + "+Modem+plug+powered+on.+Waiting+" + CONFIG.startupDelay + "+seconds+to+resume+pings.+Time:+" + timestamp);
  if (pingTimer !== null) {
    Timer.clear(pingTimer);
    pingTimer = null;
  }
